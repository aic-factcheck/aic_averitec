...
if __name__ == "__main__":
    ...
    q_score, q_all_scores = scorer.evaluate_questions_only(predictions, references)
    print_with_space("Question-only score (HU-" + scorer.metric + "):", str(q_score))
    p_score, p_all_scores = scorer.evaluate_questions_and_answers(predictions, references)
    ...
    with open("/mnt/data/factcheck/averitec-data/data_store/error_analysis_dev.jsonl", "w") as f:
        for i, (q, p) in enumerate(zip(q_all_scores, p_all_scores)):
            f.write(json.dumps({"claim_id": i, "q_score": q, "qa_score": p}) + "\n")